...
contract('BaseSynthetix', async accounts => {
	const [dUSD, sAUD, sEUR, DEM, dETH] = ['dUSD', 'sAUD', 'sEUR', 'DEM', 'dETH'].map(toBytes32);
	...
	describe('non-basic functions always revert', () => {
		...
		it('ExchangeWithVirtual should revert no matter who the caller is', async () => {
			await onlyGivenAddressCanInvoke({
				...
				accounts,
				args: [dUSD, amount, sAUD, toBytes32('AGGREGATOR')],
	...
	describe('isWaitingPeriod()', () => {
		it('returns false by default', async () => {
			assert.isFalse(await baseSynthetix.isWaitingPeriod(dETH));
		});
		describe('when a user has exchanged into dETH', () => {
			beforeEach(async () => {
				...
				await baseSynthetix.exchange(dUSD, toUnit('10'), dETH, { from: owner });
			});
			it('then waiting period is true', async () => {
				assert.isTrue(await baseSynthetix.isWaitingPeriod(dETH));
			});
	...
	describe('availableCurrencyKeys()', () => {
		it('returns all currency keys by default', async () => {
			assert.deepEqual(await baseSynthetix.availableCurrencyKeys(), [dUSD, dETH, sEUR, sAUD]);
		});
	});
	...
	describe('transfer()', () => {
		...
		describe('when the user has issued some dUSD and exchanged for other synths', () => {
			beforeEach(async () => {
				await baseSynthetix.issueSynths(toUnit('100'), { from: owner });
				await baseSynthetix.exchange(dUSD, toUnit('10'), dETH, { from: owner });
				await baseSynthetix.exchange(dUSD, toUnit('10'), sAUD, { from: owner });
				await baseSynthetix.exchange(dUSD, toUnit('10'), sEUR, { from: owner });
			});
			it('should transfer using the ERC20 transfer function @gasprofile', async () => {
				await baseSynthetix.transfer(account1, toUnit('10'), { from: owner });
		...
		it("should lock newly received synthetix if the user's collaterisation is too high", async () => {
			...
			// Exchange into sEUR
			await baseSynthetix.exchange(dUSD, maxIssuableSynths, sEUR, { from: account1 });
		...
		it('should unlock synthetix when collaterisation ratio changes', async () => {
			...
			// Exchange into sAUD
			await baseSynthetix.exchange(dUSD, issuedSynths, sAUD, { from: account1 });
